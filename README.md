...
└── resources/                      # 资源文件（可选，弹窗模板图等）
    └── ok_button.png               # 弹窗"确定"按钮模板（可选）
```
...
3. **标定"储存路径"输入框：** 将鼠标移到储存路径输入框上，按 Enter 确认
4. **标定"触发采集"按钮：** 将鼠标移到触发采集按钮上，按 Enter 确认
5. **框选"帧数显示区域"：** 在弹出的半透明覆盖层上，用鼠标拖拽框选帧数数字显示区域
...
对于每组数据（第 n 组）：
  ...
  5. OCR 识别帧数显示区域的数字
  6. 如果帧数 == 54 → 合格，进入下一组
     如果帧数 ≠ 54 或出现弹窗报错 → 删除已采集数据，重试本组
```

### 紧急停止

在自动测试过程中，**将鼠标快速移到屏幕左上角** 即可触发 PyAutoGUI 的 FailSafe 机制，立即停止程序。
...
**Q: 弹窗检测不到？**
A: 弹窗检测基于图像模板匹配，可以将常见弹窗的"确定"按钮截图保存为 `resources/ok_button.png`。如不需要弹窗检测功能，程序会自动跳过。
